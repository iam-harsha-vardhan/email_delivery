...
DF_COLS = ["Subject", "Date", "SPF", "DKIM", "DMARC",
           "Domain", "Type", "Sub ID",
...
with colA:
    if st.button("📥 Fetch Emails"):
        ...
        inbox,new_uid=fetch_emails(START_DATE,END_DATE,"inbox",
                                   st.session_state.last_uid is not None,
                                   st.session_state.last_uid,
                                   batch)

        spam,_=fetch_emails(START_DATE,END_DATE,"[Gmail]/Spam",False,None,batch)
...
with colB:
    if st.button("🗑️ Fetch Spam Only"):
        spam,_=fetch_emails(START_DATE,END_DATE,"[Gmail]/Spam",False,None,0)
        st.session_state.spam_df=pd.concat([spam,st.session_state.spam_df],ignore_index=True)

# ---------------- DISPLAY MAIN ----------------

if not st.session_state.df.empty:
    display=st.session_state.df.copy()
    display.index+=1
...
if st.session_state.show_tracking_tool and not st.session_state.df.empty:
    ...
    domain_input=st.text_area("Paste tracking domains (one per line)",height=150)
        ...
        for _,row in st.session_state.df.iterrows():
            ...
            for part in msg_data:
                ...
                if lu:
                    ...
                    for u in urls:
                        if u.startswith("http"):
                            list_unsub=u
                            tracking=urllib.parse.urlparse(u).netloc.lower()
                            break
                ...
                if tracking!="-":
                    ...
                    for l in tlinks:
                        low=l.lower()
                        if "unsub" in low: unsub=l
                        elif re.search(r'pixel|open|track',low): pixel=l
                        elif re.search(r'\.(png|jpg|jpeg|gif|svg)$',low): logo=l
