...
def get_empty_mailbox_structure():
    return {
        ...
        "df": pd.DataFrame(columns=[
            "UID", "Domain", "Subject", "From", "Message-ID", "Date", "Date_dt", "Blast/Sub ID", "Type", "SPF", "DKIM", "DMARC", "is_new"
        ]),
        "uid_date_cache": {} 
    }
...
BLAST_ID_RE = re.compile(r'^[0-9][A-Za-z0-9]{11}$')


def is_valid_blast_id(value):
    return bool(value and BLAST_ID_RE.fullmatch(value.strip()))


def get_message_id_local_part(message_id):
    if not message_id:
        return ''
    value = message_id.strip().strip('<>')
    return value.rsplit('@', 1)[0] if '@' in value else value


def split_message_id_candidates(local_part):
    if not local_part:
        return []
    parts = [local_part]
    parts.extend(re.split(r'[_:|;,<>\s]+', local_part))
...
def find_blast_ids_in_text(text):
    if not text:
        return []
    found = []
    for value in re.findall(r'(?<![A-Za-z0-9])[0-9][A-Za-z0-9]{11}(?![A-Za-z0-9])', text):
        value = value.lower()
        if value not in found:
            found.append(value)
    clean = text.strip()
    if is_valid_blast_id(clean) and clean.lower() not in found:
...
def extract_blast_id(message_id):
    ...
    def add(value):
        if not value:
            return
        value = value.strip()
        if is_valid_blast_id(value):
            value = value.lower()
            if value not in found:
                found.append(value)
        for blast in find_blast_ids_in_text(value):
            if blast not in found:
                found.append(blast)

    local_part = get_message_id_local_part(message_id)
    if not local_part:
        return '-'

    add(local_part)

    for candidate in split_message_id_candidates(local_part):
        add(candidate)
        decoded = try_base64_variants(candidate)
        if decoded:
            add(decoded)
            for nested in re.split(r'[_:|;,<>\s]+', decoded):
                if not nested:
                    continue
                add(nested)
                nested_decoded = try_base64_variants(nested)
                if nested_decoded:
                    add(nested_decoded)
...
def extract_subid_from_msg(msg):
    ...
    if msg_id_raw:
        ...
        tokens = re.split(r'[_:|;,<>\s@]+', msg_id_raw.strip('<>'))
            ...
            try:
                ...
                for u in chunk:
                    try:
                        ...
                        blast_id = extract_blast_id(message_id)
                        ...
                        results.append({
                            ...
                            "Blast/Sub ID": (sub_id if sub_id else (blast_id if blast_id != "-" else "-")), "Type": id_type,
...
for email_addr in valid_emails:
    ...
    for _, row in df_acc.iterrows():
        key = (row["Domain"], row["Subject"], row["From"], row["SPF"], row["DKIM"], row["DMARC"], row.get("Blast/Sub ID","-"))
        keys.add(key)
        if row.get("is_new", False):
            new_email_keys.add(key)
        asset_key = (row.get("Domain","-"), row.get("From","-"), row.get("Subject","-"))
        asset = asset_map.setdefault(asset_key, {"accounts": set(), "identifiers": set(), "rows": []})
        asset["accounts"].add(email_addr)
        sid = row.get("Blast/Sub ID","-")
        if sid and sid != "-":
            asset["identifiers"].add(sid)
        asset["rows"].append({
            "account": email_addr,
            "UID": row.get("UID"),
...
def get_identifier_column_name(df):
    if df.empty:
        return "Blast/Sub ID"
    values = df["Blast/Sub ID"].dropna().astype(str).str.strip()
    values = values[values != "-"]
    if values.empty:
        return "Blast/Sub ID"
    subid_mask = values.str.match(r'^(GRM|GMFP|GTC|GRTC)-', case=False, na=False)
    subid_count = int(subid_mask.sum())
    blast_count = int((~subid_mask).sum())
    if subid_count > 0 and blast_count == 0:
        return "Sub ID"
    if blast_count > 0 and subid_count == 0:
        return "Blast ID"
    return "Blast/Sub ID"


# Determine visible identifier label from the current fetched dataset.
identifier_frames = []
for em in valid_emails:
    df_tmp = st.session_state.mailbox_data[em]["df"]
    if not df_tmp.empty and "Blast/Sub ID" in df_tmp.columns:
        identifier_frames.append(df_tmp["Blast/Sub ID"])

if identifier_frames:
    identifier_series = pd.concat(identifier_frames, ignore_index=True)
    identifier_column_name = get_identifier_column_name(pd.DataFrame({"Blast/Sub ID": identifier_series}))
else:
    identifier_column_name = "Blast/Sub ID"

# ---------- TOP: Identifier table ----------
st.subheader(f"🔎 Identifier Consensus (≥ {required_accounts_count} accounts)")

identifier_rows = []
for (domain, from_val, subject), info in asset_map.items():
    if len(info["accounts"]) < required_accounts_count:
        continue
    for identifier in sorted(list(info["identifiers"])):
        identifier_rows_for_asset = [r for r in info["rows"] if (r.get("Blast/Sub ID") or "-") == identifier]
        identifier_accounts = {r["account"] for r in identifier_rows_for_asset}
        if len(identifier_accounts) < required_accounts_count:
            continue
        ...
        row = {"Domain": domain, "From": from_val, "Subject": subject, "Identifier": identifier, "Time (IST)": latest_str, "is_new": any_new}
        for em in valid_emails:
            row[em.split('@')[0]] = "✅" if em in identifier_accounts else "❌"
        identifier_rows.append((latest_dt, row))

identifier_rows_sorted = [r for _,r in sorted(identifier_rows, key=lambda x: (x[0] is None, x[0]), reverse=True)]
if identifier_rows_sorted:
    identifier_df = pd.DataFrame(identifier_rows_sorted)
    per_acc_cols = [e.split('@')[0] for e in valid_emails]
    display_cols = ["Domain","From","Subject",identifier_column_name,"Time (IST)"] + per_acc_cols + ["is_new"]
    identifier_df = identifier_df.rename(columns={"Identifier": identifier_column_name})
    identifier_df = identifier_df.reindex(columns=display_cols, fill_value="-")
    st.dataframe(identifier_df.style.apply(highlight_new_rows, axis=1), hide_index=True, column_config={"is_new": None}, use_container_width=True)
else:
    st.info(f"No identifiers that meet the threshold of {required_accounts_count} accounts with all auths passing.")
...
for (domain, from_val, subject), info in asset_map.items():
    ...
    for identifier in info["identifiers"]:
        ...
        identifier_accounts = {r["account"] for r in identifier_rows_for_asset}
        if len(identifier_accounts) >= required_accounts_count:
            qualifying_identifiers.setdefault((domain, from_val, subject), []).append(identifier)

rows = []
if all_keys:
    sorted_keys = sorted(list(all_keys), key=lambda k: (k not in new_email_keys, k[0], k[1]))
    for (domain, subject, from_val, spf, dkim, dmarc, identifier) in sorted_keys:
        latest_dt = None
        latest_str = "-"
        ...
        for em in valid_emails:
            ...
            matches = df_acc[
                (df_acc["Domain"]==domain) & (df_acc["Subject"]==subject) & (df_acc["From"]==from_val) & (df_acc["Blast/Sub ID"]==identifier)
            ]
        ...
        qual_list = qualifying_identifiers.get(qual_key, [])
        identifiers_cell = ", ".join(qual_list) if qual_list else "-"
        row = {
            "Domain": domain, "From": from_val, "Subject": subject, "Identifier (raw)": identifier,
            "Time (IST)": latest_str, "Auth": "Pass" if auth_pass_for_row else "Fail",
            "Identifiers (qualifying)": identifiers_cell,
            "is_new": (domain, subject, from_val, spf, dkim, dmarc, identifier) in new_email_keys,
            "Date_dt_sort": latest_dt
        }
        for em in valid_emails:
            row[em.split('@')[0]] = "✅" if (domain, subject, from_val, spf, dkim, dmarc, identifier) in email_presence_map[em] else "❌"
        rows.append(row)
    ...
    if not presence_df.empty:
        ...
        display_cols = ["Domain","From","Subject",f"{identifier_column_name} (raw)","Time (IST)"] + per_account_cols + [f"{identifier_column_name}s (qualifying)","Auth","is_new"]
